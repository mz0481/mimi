...
def check_keyword(mention_return_length, mention_return): 
    for i in range(mention_return_length-1, -1, -1): # 최신 멘션부터 불러오기 때문에 거꾸로 순회
        ...
        reply_content = "미니스타가 반짝반짝 빛나며 말했다." + '\n\n' + "다시 말씀해주세요, 반짝!" # 초기 답멘 내용. 키워드 오류 때문에 다음 함수로 넘어가지 않으면 이 내용이 답멘으로 출력된다.
...
def roll_dice(mention_keyword):
    ...
    second_keyword = mention_keyword[0].strip() # 두번째 키워드를 보정한다. strip()은 앞뒤 공백을 지우는 함수다. (ex: ' 1d2 ' 였다면 '1d2'로 만든다)
    ...
    if len(dice_info) == 2 and (dice_info[1] == '100' or dice_info[1] == '20'): # D 앞뒤로 숫자 있나확인
        try:
